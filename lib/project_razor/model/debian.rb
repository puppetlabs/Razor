...
module ProjectRazor
  module ModelTemplate
    # Root Model object
    # @abstract
    class Debian < ProjectRazor::ModelTemplate::Base
      ...
      def initialize(hash)
        ...
        @name = "debian"
        @description = "Debian Generic Model"
        # Metadata vars
        @hostname_prefix = nil
      ...
      def template_filepath(filename)
        raise ProjectRazor::Error::Slice::InternalError, "must provide Debian version." unless @osversion

        filepath = File.join(File.dirname(__FILE__), "debian/#{@osversion}/#{filename}.erb")
      end
