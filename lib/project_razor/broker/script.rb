...
module ProjectRazor::BrokerPlugin
  ...
  class Script < ProjectRazor::BrokerPlugin::Base
    ...
    def agent_hand_off(options = {})
      ...
      begin
        Net::SCP.start(options[:ipaddress], options[:username], {:password => options[:password], :user_known_hosts_file => '/dev/null'}) do |scp|
          # Install Script
        ...
        Net::SSH.start(options[:ipaddress], options[:username], {:password => options[:password], :user_known_hosts_file => '/dev/null'}) do |session|
          ...
          @output << session.exec!("#{options[:script_path]} #{options[:metadata_path]} 2>&1 | tee #{options[:log_path]}")
        end
